...
then set profile_check = "correct" ;
elseif (exp>16 and role="President")
then set profile_check = "correct" ;
else  set profile_check = "not correct" ;
end if ;
return(profile_check) ;
end &&
select concat(first_name,' ',last_name) as employee_name, exp , role , job_profile(exp, role) from data_science_team;
...
add index firstname_index(first_name);
select * from emp_record_table 
